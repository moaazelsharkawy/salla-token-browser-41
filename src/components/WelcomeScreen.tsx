...
/**
 * Compact, English-only welcome screen.
 * Exported as a named export `WelcomeScreen` and also as default
 * so either `import { WelcomeScreen } ...` or `import WelcomeScreen ...` works.
 */
export const WelcomeScreen = ({ onComplete }: WelcomeScreenProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-gradient-to-br from-primary via-primary/90 to-secondary flex items-start pt-12 justify-center p-4">
      {/* Dimmed grid background (reduced density for compact look) */}
      <div className="absolute inset-0 opacity-16">
        <div className="grid grid-cols-8 gap-1 h-full w-full">
          {[...Array(96)].map((_, i) => (
            <div
              key={i}
              className={`border border-white/12 ${i % 7 === 0 ? 'bg-primary/20' : i % 11 === 0 ? 'bg-secondary/20' : ''}`}
              style={{
      ...
      <div className="relative z-10 w-full max-w-md bg-transparent text-white rounded-xl p-4 shadow-lg max-h-[68vh] overflow-y-auto">
        <div className="flex flex-col items-center gap-4">
          ...
          <div className="w-28 h-28 relative flex items-center justify-center">
            <div
              className="absolute inset-0 rounded-full bg-gradient-to-r from-primary via-accent to-secondary animate-spin"
              style={{ animationDuration: '8s' }}
            />
            <div
              className="absolute inset-2 rounded-full bg-gradient-to-l from-secondary via-primary to-accent animate-spin"
          ...
          <div className="text-center">
            <h1 className="text-2xl font-semibold leading-tight">{steps[currentStep].title}</h1>
            <p className="text-sm text-white/85 mt-2">{steps[currentStep].subtitle}</p>
            {currentStep === 1 && (
