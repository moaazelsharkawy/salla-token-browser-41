...
// تمت إزالة مكتبة @react-three/postprocessing غير المدعومة، واستبدال التأثيرات بمكونات three.js أساسية
...
          color="#00e6ff"
          emissive="#00d9ff"
          emissiveIntensity={0.8} // زيادة شدة الإضاءة الذاتية لتعويض الـ Bloom
          metalness={0.6}
          roughness={0.2}
        />
      </mesh>
...
function Edge({ start, end, thickness, seed }) {
...
      // Use fast flicker (t * 5) to look like high-frequency data
      const intensity = 0.5 + 0.5 * Math.sin(t * 5 + seed); 
      materialRef.current.emissiveIntensity = intensity * 0.4; // زيادة الشدة
...
          color="#00aaff" 
          emissive="#00ccff"
          emissiveIntensity={0.4} // زيادة الإضاءة
          metalness={0.5} 
          roughness={0.25} 
          transparent 
          opacity={0.7}
...
function ParticleField({ count = 160 }) {
  const pointsRef = useRef(null);

  const positions = useMemo(() => {
    const arr = new Float32Array(count * 3);
    for (let i = 0; i < count; i++) {
      const r = Math.random() * 5 + 1; // slightly larger field
      const theta = Math.random() * Math.PI * 2;
      const phi = Math.acos((Math.random() * 2) - 1);
      const x = r * Math.sin(phi) * Math.cos(theta);
      const y = r * Math.sin(phi) * Math.sin(theta);
      const z = r * Math.cos(phi) * 0.8; // flatten Z a bit
      arr[i * 3] = x;
      arr[i * 3 + 1] = y;
      arr[i * 3 + 2] = z;
    }
...
    const radius = 2.6; // Increased radius for better spread
    const generatedNodes = [];
    
    for (let i = 0; i < nodeCount; i++) {
      // Generate coordinates with slight bias towards center/periphery
      let x = rand() * 2 - 1;
      let y = rand() * 2 - 1;
      let z = rand() * 2 - 1;
      const v = new THREE.Vector3(x, y, z).normalize().multiplyScalar(rand() * radius);
      if (rand() < 0.3) v.multiplyScalar(0.7 + rand() * 0.5); // Cluster bias
...
  onComplete,
  nodeCount = 18, // Increased node count
  connectionProbability = 0.35, // Increased connection probability
  thickness = 0.025,
  seed = 42,
}) => {
...
          // إضافة الضباب يدويًا لتعويض تأثير Vignette والعمق
          onCreated={({ scene }) => {
            scene.fog = new THREE.FogExp2(0x0a0a1a, 0.05); // لون ضبابي داكن
          }}
        >
...
          {/* تم حذف EffectComposer ومؤثرات ما بعد المعالجة */}
        
        </Canvas>
      </div>
...
// Required for the React Sandbox
